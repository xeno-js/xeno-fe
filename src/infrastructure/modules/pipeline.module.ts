...
export class PipelineModule {
     ...
     */
    public static async create(config: PipelineConfig, logger: Optional<ILogger>, cache: Optional<ICache>, cahceKeyBuilder: Optional<ICacheKeyBuilder>): Promise<IClientMediator> {
        const { Guards } = await import('@xeno-js/shared')
        if(!Guards.isDefined(logger))
            throw new Error('At least must provide a logger service')
        if(!Guards.isDefined(cache))
            throw new Error('At least must provide a cache service')
        ...
        return new ClientMediator(new CompositePipeline(pipelines), new CompositePipeline(queryPipelines))
    }
}